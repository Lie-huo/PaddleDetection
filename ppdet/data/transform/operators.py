...
class NormalizeImage(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        '''
        print('im', type(im))
        debug_t = sample['gt_bbox'].astype(np.int32)
        debug_t = debug_t.reshape(-1, 4, 2)
        for ii in range(debug_t.shape[0]):
            pt_lst = debug_t[ii, :, :]
            for i in range(4):
                tx1,ty1 = (pt_lst[i][0], pt_lst[i][1])
                tx2,ty2 = (pt_lst[(i+1)%4][0], pt_lst[(i+1)%4][1])
                print(tx1, ty1, tx2, ty2)
                cv2.rectangle(im, (tx1, ty1), (tx2, ty2), (255,0,0))
...
@register_op
class RandomFlipRbox(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(RandomFlipRbox, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply_bbox(self, bbox, width):
        oldx1 = bbox[:, 0::2].copy()
        oldx2 = bbox[:, 1::2].copy()
        bbox[:, 0::2] = width - oldx2
        bbox[:, 1::2] = width - oldx1
        return bbox
...
@register_op
class Rbox2Ploy(BaseOperator):
    """
    Convert poly format to rbbox format.
    """

    def __init__(self):
        super(Rbox2Ploy, self).__init__()
